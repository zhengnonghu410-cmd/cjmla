...
class Menu:
    def __init__(self, screen, dashboard, level, sound):
        ...
        self.loadSettings(asset_path("./settings.json"))
    ...
    def loadSettings(self, url):
        try:
            ...
            self.sound.allowSFX = False
            self.sfx = False
            self.saveSettings(asset_path("./settings.json"))
    ...
    def loadLevelNames(self):
        files = []
        res = []
        for r, d, f in os.walk(asset_path("./levels")):
            for file in f:
    ...
    def _handleConfirm(self):
        """Handle confirm/select action (Enter key or tap)."""
        if self.inChoosingLevel:
            self.inChoosingLevel = False
    ...
    def _handleClick(self, mx, my):
        """Handle touch/mouse click on menu items."""
        if self.inChoosingLevel:
            self._handleLevelChooserClick(mx, my)
            return

        # Hit-test menu items (text positions from drawMenu/drawSettings)
        if not self.inSettings:
            # Main menu: CHOOSE LEVEL (y=280), SETTINGS (y=320), EXIT (y=360)
            menu_items = [
                pygame.Rect(145, 265, 250, 40),   # CHOOSE LEVEL
                pygame.Rect(145, 305, 250, 40),   # SETTINGS
                pygame.Rect(145, 345, 250, 40),   # EXIT
            ]
            for i, rect in enumerate(menu_items):
                if rect.collidepoint(mx, my):
                    self.state = i
                    self._handleConfirm()
                    return
        else:
            # Settings: MUSIC (y=280), SFX (y=320), BACK (y=360)
            settings_items = [
                pygame.Rect(145, 265, 250, 40),   # MUSIC toggle
                pygame.Rect(145, 305, 250, 40),   # SFX toggle
                pygame.Rect(145, 345, 250, 40),   # BACK
            ]
            for i, rect in enumerate(settings_items):
                if rect.collidepoint(mx, my):
                    self.state = i
                    self._handleConfirm()
                    return

    def _handleLevelChooserClick(self, mx, my):
        """Handle click in the level chooser grid."""
        offset = 75
        # Row 0: i=0..2, Row 1: i=3..5
        for i in range(self.levelCount):
            col = i % 3
            row = i // 3
            bx = 175 * col + offset
            by = 55 if row == 0 else 210
            cell_rect = pygame.Rect(bx, by, 125, 75)
            if cell_rect.collidepoint(mx, my):
                self.currSelectedLevel = i + 1
                self._handleConfirm()
                return
